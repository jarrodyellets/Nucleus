    ...
    const comment = comments.map((comment, i) => {
      return (
        <div className="commentsMainWrapper" key={this.props.id} onClick={() => {this.props.handleModal(true, comment); this.props.handleSelectedPost(comment.postID)}}>
          ...
          <LikesContainer post={comment} id={this.props.user.id} handleLike={this.handleLike} handleDislike={this.handleDislike} handleTrigger={this.props.handleTrigger} />
        </div>
      )
    })
    return (
...
export default connect(mapStateToProps, {addLike, disLike})(DisplayComment);