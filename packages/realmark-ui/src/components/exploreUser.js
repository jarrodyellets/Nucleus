    ...
    <div className="exploreInnerWrapper">
      <div className="exploreImgWrapper">
        <img
          ...
          alt="Profile"
          src={
            props.user.imageURL ? props.user.imageURL : 'https://www.jarrodyellets.com/images/profilePlaceholder.png'
          }
        />
      </div>
      <div className="profileCardInfoWrapper">
        <div className="profileCardNameWrapper">
          <div
            className="exploreFullName"
            onClick={() => {
              props.handleSearch(props.user.userName);
            }}>
            {props.user.firstName} {props.user.lastName}
          </div>
          {props.userData.id === props.user.id ? null : props.userData.following.indexOf(props.user.id) === -1 ? (
            <button
              onClick={() => {
                props.handleFollow(props.user.id);
              }}
              className="profileCardFollowButton">
              Follow
            </button>
          ) : (
            <button
              onClick={() => {
                props.handleUnFollow(props.user.id);
              }}
              className="profileCardFollowButton">
              Unfollow
            </button>
        ...
        <div className="profileCardContactWrapper">
          <div className="profileCardContact">
            <i className="far fa-envelope fa-icon" /> {props.user.email}
          </div>
          <div className="profileCardLocation">
            <i className="fas fa-map-pin fa-icon" /> {props.user.location}
          </div>
        </div>
